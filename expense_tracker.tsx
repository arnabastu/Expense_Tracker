...
// Map of category emojis for beautiful visual display
...
export default function App() {
  ...
  // Clean-Slate Core Data State (No hardcoded historical data of other people)
  ...
  // Starting Balance Offset (Customizable)
  ...
  const [selectedDayIndex, setSelectedDayIndex] = useState(new Date().getDate() - 1); // Select current day of the month
  const [selectedDonutCategory, setSelectedDonutCategory] = useState('');

  // Input states for Add/Edit Transactions
  const [txForm, setTxForm] = useState({ id: '', title: '', category: 'Food', amount: '', type: 'expense', date: new Date().toISOString().split('T')[0], time: '12:00 PM' });
  // Input states for Savings Goal
  ...
  // Dynamic Calculations for current user metrics
  const metrics = useMemo(() => {
    const today = new Date().toISOString().slice(0, 10);
    const categoryTotals: Record<string, number> = {};
    const totals = transactions.reduce<{ income: number; expense: number; today: number }>((acc, { type, amount, category, date }) => {
    ...
    const totalExpenseAggregate = Object.values(categoryTotals).reduce((a, b) => a + b, 0);
    return {
  ...
  useEffect(() => {
    ...
    setSelectedDonutCategory(keys[0] || '');
  }, [metrics.categoryTotals]);

  // Action: Add/Edit Transaction
  ...
  // Action: Set Balance Manual Override
  ...
  return (
    <div className="min-h-screen bg-[#131313] text-[#e5e2e1] font-sans flex flex-col items-center justify-start py-0 md:py-8 antialiased">
      ...
      {/* Scrollbar overrides for ultra clean viewport navigation */}
      <style>{`
      ...
      {/* Dynamic Toast System with Frosted glass */}
      {toastMessage && (
      ...
      <div className="w-full max-w-md bg-[#131313] md:rounded-[40px] md:shadow-[0_24px_60px_rgba(0,0,0,0.85)] border-x border-[#2a2a2a]/60 md:border-t h-screen md:h-[880px] flex flex-col justify-between relative overflow-hidden">
        ...
        {/* Decorative Hardware Notch Simulation */}
        ...
        <header className="px-6 pt-6 pb-2 flex items-center justify-between shrink-0 bg-[#131313] z-10">
          ...
          <div className="flex items-center space-x-4">
            <button 
              onClick={() => triggerToast('Ready for new transactions')} 
              className="p-2 rounded-full hover:bg-white/5 transition-all relative active:scale-95 border border-white/5"
            >
              <Icons.Bell />
              <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-[#78dc77] rounded-full animate-ping"></span>
            </button>
            <div className="w-10 h-10 rounded-full border border-[#78dc77] flex items-center justify-center bg-white/5 text-sm font-bold text-[#78dc77] uppercase">
              ME
            </div>
          </div>
        </header>

        {/* --- SCREEN CONTENT AREA - SCROLLABLE MAIN CONTENT (Feature 3) --- */}
        <main className="flex-1 px-6 overflow-y-auto pb-28 no-scrollbar scroll-smooth">
          ...
          {currentTab === 'dashboard' && (
            <div className="space-y-4 animate-fadeIn">
              ...
              {/* WALLET SETUP PROMPT IF NO DATA SET */}
              {startingBalance === 0 && transactions.length === 0 && (
                <div className="bg-[#78dc77]/10 border border-[#78dc77]/20 p-5 rounded-3xl space-y-3 animate-pulse">
                  ...
                  <button
                    ...
                    className="bg-[#78dc77] text-black px-4 py-2 rounded-xl text-xs font-bold shadow-md hover:bg-opacity-95 transition-all"
                  >
              ...
              {/* CURRENT BALANCE GLASS CARD (Feature 1 Modification) */}
              <div className={`${glassClass} p-6 rounded-3xl relative overflow-hidden`}>
                ...
                <div className="flex justify-between items-center">
                  ...
                  {/* Plus icon to modify balance */}
                  <button 
                    onClick={() => {
                      setNewBalanceInput(metrics.balance.toString());
                      setShowBalanceModal(true);
                    }}
                    title="Set Custom Balance"
                    className="p-1 rounded-full bg-white/5 hover:bg-white/10 text-[#78dc77] transition-all active:scale-90 border border-white/5"
                  >
                ...
                <div className="flex items-baseline space-x-2 mt-2">
                  <span className="text-4xl font-extrabold tracking-tight font-mono text-[#78dc77]">₹ {metrics.balance.toLocaleString()}</span>
              ...
              {/* STATS ROW 1 - GLASS (Feature 1 Modification) */}
              <div className="grid grid-cols-2 gap-3">
                <div className={`${glassClass} p-4 rounded-2xl`}>
                  <p className="text-xs text-[#becab9] font-medium">Today's Expense</p>
                  <p className="text-xl font-bold font-mono text-white mt-1">₹{metrics.todaySpending.toLocaleString()}</p>
                </div>
                <div className={`${glassClass} p-4 rounded-2xl border-l-2 border-l-[#78dc77]`}>
                  ...
                  <p className="text-xl font-bold font-mono text-white mt-1">₹{metrics.expenseSum.toLocaleString()}</p>
                </div>
              </div>

              {/* STATS ROW 2 - GLASS (Feature 1 Modification) */}
              <div className="grid grid-cols-2 gap-3">
                <div className={`${glassClass} p-4 rounded-2xl border-l-2 border-l-[#78dc77]`}>
                  <p className="text-xs text-[#becab9] font-medium">Income Streams</p>
                  <p className="text-xl font-bold font-mono text-[#78dc77] mt-1">₹{metrics.incomeSum.toLocaleString()}</p>
                </div>
                <div className={`${glassClass} p-4 rounded-2xl border-l-2 border-l-[#ff6b67]`}>
                  ...
                  <p className="text-xl font-bold font-mono text-[#ff6b67] mt-1">₹{metrics.expenseSum.toLocaleString()}</p>
                </div>
              </div>

              {/* BUDGET PROGRESS - GLASS (Feature 1 Modification) */}
                ...
                <div className="w-full bg-black/40 h-2 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-500 ${
                      metrics.expenseSum > budgetLimit ? 'bg-red-500' : 'bg-[#78dc77]'
                    }`} 
                    style={{ width: `${Math.min(100, budgetLimit > 0 ? (metrics.expenseSum / budgetLimit) * 100 : 0)}%` }}
                  ></div>
                </div>

                <div className="flex justify-between items-center mt-3 text-xs text-[#becab9]">
                  <span className="font-mono">Spent: ₹{metrics.expenseSum}</span>
                  <span className="font-mono">Cap: ₹{budgetLimit}</span>
                </div>
              </div>

              {/* TOP SPENDING CATEGORY - GLASS (Feature 1 Modification) */}
              <div className={`${glassClass} p-4 rounded-2xl flex items-center justify-between`}>
                ...
                <div className="flex items-center space-x-1">
                  <span className="text-base font-bold text-[#ff6b67] font-mono">
                    ₹{Object.keys(metrics.categoryTotals).length > 0 
                      ? Math.max(...Object.values(metrics.categoryTotals))
                      : 0}
                  </span>
                  <Icons.ChevronRight />
                </div>
                ...
                <div className="flex justify-between items-center px-1">
                  ...
                  {transactions.length > 0 && (
                    <button 
                      ...
                      className="text-xs font-semibold text-[#78dc77] hover:underline"
                    >
                      View All
                    </button>
                  )}
          ...
          {currentTab === 'transactions' && (
            <div className="space-y-4 animate-fadeIn no-scrollbar">
              ...
              {/* SEARCH BAR - GLASS (Feature 1 Modification) */}
              <div className="relative">
                ...
                {searchQuery && (
                  <button 
                    ...
                    className="absolute right-4 top-3 text-xs bg-white/10 text-[#becab9] rounded-lg px-2 py-1"
                  >
                    Clear
                  </button>
                )}
              ...
              {/* TRANSACTIONS RENDER GRID */}
              <div className="space-y-4 no-scrollbar">
                {transactions.length > 0 ? (
                  txGroups.map(({ name, type }) => {
                    const filtered = transactions.filter(tx => {
                      const query = searchQuery.toLowerCase();
                      return (tx.title.toLowerCase().includes(query) || tx.category.toLowerCase().includes(query))
                    ...
                    return (
                      <div key={name} className="space-y-2 no-scrollbar">
                        <div className="flex justify-between items-center text-[10px] font-bold tracking-widest text-[#becab9] uppercase px-1">
                          <span>{name}</span>
                          <span className={type === 'expense' ? 'text-[#ff6b67]' : 'text-[#78dc77]'}>
                            {fmt(totalSum)}
                          </span>
                        </div>
                  ...
                  <div className="text-center py-16 text-[#becab9]">
                    ...
                    <p className="text-xs mt-1 text-white/50 max-w-xs mx-auto">
                      All your expenses and incomes will dynamically appear here when you create them.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-4 animate-fadeIn">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className={`${glassClass} p-4 rounded-2xl`}>
                  ...
                  <p className="text-2xl font-bold font-mono text-[#78dc77] mt-1">
                    ₹{transactions.filter(t => t.type === 'expense').length > 0
                      ? Math.round(metrics.expenseSum / 30)
                      : 0}
                  </p>
                ...
                <div className={`${glassClass} p-4 rounded-2xl`}>
                  ...
                  <p className="text-[10px] text-[#becab9] mt-2 font-mono truncate">
                    {transactions.length > 0 
                      ? transactions.reduce((max, t) => t.amount > max.amount ? t : max, transactions[0]).title 
                      : 'No items'}
                  </p>
                </div>
              </div>

              {/* HIGHEST SPENDING DAY - GLASS */}
              <div className={`${glassClass} p-4 rounded-2xl flex justify-between items-center`}>
                <div>
                  ...
                  <p className="text-lg font-bold text-white mt-1">
                    ₹{Math.max(...juneDailyExpenses) > 0 ? Math.max(...juneDailyExpenses) : 0}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-[#78dc77] font-semibold">
                    {Math.max(...juneDailyExpenses) > 0 
                      ? `June ${juneDailyExpenses.indexOf(Math.max(...juneDailyExpenses)) + 1}` 
                      : 'No Data'}
                  </p>
              ...
              {/* FEATURE 4: BAR GRAPH INSIGHTS - DYNAMIC June 1-30 */}
              <div className={`${glassClass} p-5 rounded-3xl space-y-3`}>
                <div className="flex justify-between items-center">
                  ...
                  <div className="bg-black/40 border border-[#78dc77]/40 px-2.5 py-1 rounded-xl text-right">
                    ...
                    <p className="text-xs font-bold text-[#78dc77] font-mono">₹{juneDailyExpenses[selectedDayIndex] || 0}</p>
                  </div>
                </div>

                {/* Day bar visual graph */}
                    ...
                    {juneDailyExpenses.map((val, idx) => {
                      ...
                      return (
                        <g 
                          ...
                          onTouchStart={() => setSelectedDayIndex(idx)}
                        >
                          ...
                          <rect 
                            ...
                            height={barHeight}
                            rx={2}
                            fill={val > 0 ? (isSelected ? "#78dc77" : "rgba(120, 220, 119, 0.4)") : (isSelected ? "#78dc77" : "rgba(255,255,255,0.05)")}
                            className="transition-all duration-150 group-hover:fill-[#78dc77]"
                          />
                          {isSelected && (
                            <circle cx={x + barWidth / 2} cy={y - 5} r="2.5" fill="#78dc77" />
                          )}
                        </g>
                      );
                    })}
              ...
              {/* FEATURE 5: PIE/DONUT CHART WITH FULL RESPONSIVE TOGGLE */}
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-around gap-4">
                    ...
                    <div className="relative w-36 h-36 flex-shrink-0">
                      <svg viewBox="0 0 36 36" className="w-full h-full transform -rotate-90">
                        ...
                        {/* Compute and draw active slices dynamically */}
                        {(() => {
                          let accumulatedPercentage = 0;
                          return Object.entries(metrics.categoryTotals).map(([cat, amount], idx) => {
                      ...
                      {/* Display content inside the donut based on selected category */}
                      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none p-1 text-center">
                        <span className="text-[9px] font-bold tracking-widest text-[#becab9] uppercase truncate max-w-[80px]">
                          {selectedDonutCategory || 'Expense'}
                        </span>
                        <span className="text-xs font-extrabold text-white font-mono mt-0.5">
                          ₹{metrics.categoryTotals[selectedDonutCategory] || 0}
                        </span>
                        <span className="text-[10px] font-semibold text-[#78dc77] font-mono">
                          {metrics.totalExpenseAggregate > 0 
                            ? `${Math.round(((metrics.categoryTotals[selectedDonutCategory] || 0) / metrics.totalExpenseAggregate) * 100)}%`
                            : '0%'}
                        </span>
                      </div>
                    </div>

                    {/* Color legends matching the dynamic segments */}
                      ...
                      {Object.entries(metrics.categoryTotals).map(([cat, amount], idx) => {
                        ...
                        return (
                          <button 
                            key={cat}
                            onClick={() => setSelectedDonutCategory(cat)}
                            className={`flex items-center justify-between w-full sm:w-36 px-2.5 py-1.5 rounded-xl transition-all border ${
                              selectedDonutCategory === cat ? 'bg-white/5 border-white/15' : 'border-transparent'
                            }`}
                          >
            ...
            <div className="space-y-4 animate-fadeIn">
              ...
              <div className={`${glassClass} p-6 rounded-3xl`}>
                ...
                <div className="flex justify-between items-baseline mt-2">
                  ...
                  <span className="text-sm font-semibold text-[#78dc77] font-mono">
                    of ₹{goals.reduce((sum, g) => sum + g.target, 0).toLocaleString()} Target
                  </span>
                </div>
                <div className="w-full bg-black/40 h-2.5 rounded-full mt-4 overflow-hidden">
                  <div 
                    className="h-full bg-[#78dc77] rounded-full transition-all duration-500" 
                    style={{ 
                      width: `${Math.min(100, goals.reduce((sum, g) => sum + g.target, 0) > 0 
                        ? (goals.reduce((sum, g) => sum + g.current, 0) / goals.reduce((sum, g) => sum + g.target, 0)) * 100 
                        : 0)}%` 
                    }}
                  ></div>
                </div>
                ...
                {goals.length > 0 ? (
                  goals.map((goal) => {
                    ...
                    return (
                      <div key={goal.id} className={`${glassClass} p-5 rounded-2xl space-y-4`}>
                        ...
                        <div className="flex justify-between text-xs font-mono text-[#becab9]">
                        ...
                        <div className="flex justify-between items-center pt-1">
                          <span className="text-xs font-semibold text-[#becab9] font-mono">
                            {isFinished ? '🎉 Goal Achieved!' : `₹${goal.target - goal.current} remaining`}
                          </span>
                              ...
                              {fundingGoalId === goal.id ? (
                                <div className="flex items-center space-x-1 animate-fadeIn">
                                  <input 
                                    ...
                                    className="w-16 bg-white/5 border border-[#78dc77]/40 rounded-xl px-2 py-1 text-xs text-white font-mono focus:outline-none"
                                  />
                                  <button 
                                    onClick={() => handleAddFunds(goal.id)}
                                    className="bg-[#78dc77] text-black px-2.5 py-1 rounded-lg text-[10px] font-bold"
                                  >
                                    Add
                                  </button>
                                  <button 
                                    onClick={() => setFundingGoalId(null)}
                                    className="bg-white/10 text-white px-2 py-1 rounded-lg text-[10px] font-bold"
                                  >
                                    X
                                  </button>
                                </div>
                                ...
                                <button 
                                  onClick={() => { setFundingGoalId(goal.id); setFundingAmount(''); }}
                                  className="bg-white/5 hover:bg-white/10 text-white px-3 py-1.5 rounded-xl text-xs font-semibold transition-all border border-white/5"
                                >
                                  Add Funds
                                </button>
                              )}
                  ...
                  <div className="text-center py-12 text-[#becab9] bg-white/5 border border-dashed border-white/5 rounded-3xl">
                    ...
                    <p className="text-xs text-white/50 mt-1 max-w-xs mx-auto">
                      Define milestones like gadgets, setup plans, or holidays to stay organized.
                    </p>
                  </div>
                )}
              </div>
              ...
              <button 
                ...
                className="w-full bg-[#78dc77] hover:bg-opacity-90 text-[#131313] py-4 rounded-2xl font-bold transition-all shadow-lg active:scale-95 flex items-center justify-center space-x-2"
              >
                <span>➕</span>
          ...
          {currentTab === 'settings' && (
            <div className="space-y-4 animate-fadeIn">
              ...
              <div className={`${glassClass} p-5 rounded-2xl flex items-center space-x-4`}>
                <div className="w-14 h-14 rounded-full border border-[#78dc77] bg-white/5 text-[#78dc77] font-bold text-lg flex items-center justify-center uppercase">
                  ME
                </div>
                <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex space-x-2">
                    <input 
                      ...
                      className="bg-black/20 border border-white/5 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:border-[#78dc77] flex-1 font-mono"
                    />
                    <button 
                      onClick={() => triggerToast('Cap limit updated')} 
                      className="bg-[#78dc77] text-[#131313] px-4 py-2 rounded-xl text-xs font-bold"
                    >
                      Apply
                    </button>
                  </div>
              ...
              <div className={`${glassClass} p-5 rounded-2xl space-y-4`}>
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map((cat) => (
                    <span key={cat} className="bg-white/5 text-white px-3 py-1.5 rounded-full text-xs font-semibold flex items-center space-x-1.5 border border-white/5 shadow-sm">
                  ...
                  <button 
                    ...
                    className="bg-black/20 border-dashed border border-[#78dc77] text-[#78dc77] px-3 py-1.5 rounded-full text-xs font-bold hover:bg-white/5"
                  >
                    + Add Category
                  </button>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button 
                  onClick={() => triggerToast('CSV generated successfully')}
                  className="bg-white/5 text-white p-3 rounded-xl border border-white/5 text-xs font-bold transition-all hover:bg-white/10 shadow-lg"
                >
                  📤 Export CSV
                </button>
                <button 
                  ...
                  className="bg-red-950/40 border border-red-500/20 text-red-200 p-3 rounded-xl text-xs font-bold transition-all hover:bg-red-950/60"
                >
                  🧹 Clear Data
                </button>
              </div>
        ...
        <div className="absolute bottom-24 right-6 z-20">
          <button 
            ...
            className="w-14 h-14 bg-[#78dc77] hover:bg-opacity-95 rounded-2xl shadow-[0_8px_32px_rgba(120,220,119,0.4)] flex items-center justify-center transition-all duration-300 transform active:scale-90"
          >
            <Icons.Plus />
          </button>
        </div>

        {/* --- CONSTANT BOTTOM GLASS NAVIGATION TAB BAR (Feature 2 & 3 Optimization) --- */}
        <nav className="h-20 bg-[#1e1e1e]/70 backdrop-blur-lg border-t border-white/5 flex items-center justify-around px-2 rounded-b-[40px] shrink-0 z-10 shadow-[0_-8px_32px_0_rgba(0,0,0,0.5)]">
          ...
          {tabItems.map(({ id, Icon, label }) => (
            <button
              ...
              className={`flex flex-col items-center justify-center w-[74px] h-14 rounded-2xl transition-all ${
                currentTab === id ? 'text-[#78dc77] bg-white/5' : 'text-[#becab9] hover:text-white'
              }`}
            >
              <Icon />
        ...
        {/* --- MODAL A: ADJUST CURRENT BALANCE --- */}
        {showBalanceModal && (
            ...
            <div className="bg-[#1c1b1b] w-full rounded-t-[30px] border-t border-white/10 p-6 space-y-4 max-h-[90%] overflow-y-auto animate-slideUp">
              <div className="flex justify-between items-center border-b border-white/5 pb-4">
                ...
                <button 
                  ...
                  className="p-1 text-white bg-white/5 rounded-full text-xs w-6 h-6 flex items-center justify-center font-bold"
                >
                  ✕
                </button>
              </div>
              ...
              <form onSubmit={handleUpdateBalance} className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-[#2c2c2c] border border-white/10 rounded-xl py-3 px-4 text-white text-lg font-mono focus:outline-none focus:border-[#78dc77]"
                    required
                  />
                  <p className="text-[11px] text-[#becab9] mt-2 leading-relaxed">
                    Set your starting cash balance. Any transactions you create subsequently will flow against this amount dynamically.
                  </p>
                </div>
                ...
                <div className="pt-2 flex space-x-2">
                  <button 
                    type="button" 
                    onClick={() => setShowBalanceModal(false)}
                    className="w-1/3 bg-white/5 text-white py-3 rounded-xl font-bold text-sm"
                  >
                    Cancel
                  </button>
                  <button 
                    type="submit" 
                    className="w-2/3 bg-[#78dc77] text-black py-3 rounded-xl font-bold text-sm shadow-md"
                  >
                    Update Balance
                  </button>
                </div>
            ...
            <div className="bg-[#1c1b1b] w-full rounded-t-[30px] border-t border-white/10 p-6 space-y-4 max-h-[90%] overflow-y-auto animate-slideUp">
              ...
              <div className="flex justify-between items-center border-b border-white/5 pb-4">
                <h3 className="text-base font-bold text-white">
                  {txForm.id ? 'Edit Transaction' : `Add ${addModalType === 'expense' ? 'Expense' : 'Income'}`}
                </h3>
                
                {!txForm.id && (
                  <div className="flex bg-black/40 p-1 rounded-xl border border-white/5">
                    <button 
                      onClick={() => { setAddModalType('expense'); setTxForm(p => ({ ...p, type: 'expense' })); }}
                      className={`px-3 py-1 rounded-lg text-xs font-semibold ${addModalType === 'expense' ? 'bg-[#ff6b67] text-white' : 'text-[#becab9]'}`}
                    >
                      Expense
                    </button>
                    <button 
                      onClick={() => { setAddModalType('income'); setTxForm(p => ({ ...p, type: 'income', category: 'Salary' })); }}
                      className={`px-3 py-1 rounded-lg text-xs font-semibold ${addModalType === 'income' ? 'bg-[#78dc77] text-black' : 'text-[#becab9]'}`}
                    >
                      Income
                    </button>
                  </div>
              ...
              <form onSubmit={handleSaveTransaction} className="space-y-4">
                ...
                <div>
                  ...
                  <input 
                    type="text" 
                    placeholder="e.g. Swiggy checkout, Freelance pay"
                    value={txForm.title}
                    onChange={(e) => setTxForm(p => ({ ...p, title: e.target.value }))}
                    className="w-full bg-[#2c2c2c] border border-white/10 rounded-xl py-3 px-4 text-white text-sm focus:outline-none focus:border-[#78dc77]"
                    required
                  />
                </div>
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-[#2c2c2c] border border-white/10 rounded-xl py-3 px-4 text-white text-sm focus:outline-none focus:border-[#78dc77] font-mono"
                  />
                </div>
                ...
                <div className="pt-4 flex space-x-2">
                  <button 
                    type="button" 
                    onClick={() => setShowAddModal(false)}
                    className="w-1/3 bg-white/5 text-white py-3.5 rounded-xl font-bold text-sm"
                  >
                    Cancel
                  </button>
                  <button 
                    type="submit" 
                    className="w-2/3 bg-[#78dc77] text-black py-3.5 rounded-xl font-bold text-sm shadow-md"
                  >
                    Save Transaction
                  </button>
                </div>
        ...
        {/* --- MODAL 2: TRANSACTION DETAILS OVERLAY --- */}
          ...
          <div className="absolute inset-0 bg-black/60 backdrop-blur-md flex items-end justify-center z-40 transition-all">
            <div className="bg-[#1c1b1b] w-full rounded-t-[30px] border-t border-white/10 p-6 space-y-6 animate-slideUp">
              ...
              <div className="text-center py-4 space-y-2">
                <div className="w-16 h-16 mx-auto rounded-full bg-white/5 border border-white/5 flex items-center justify-center text-3xl">
                  {categoryEmojis[selectedTx.category] || "🏷️"}
                </div>
                ...
                <p className={`text-3xl font-extrabold font-mono mt-4 ${selectedTx.type === 'income' ? 'text-[#78dc77]' : 'text-[#ff6b67]'}`}>
              ...
              <div className="bg-black/40 p-4 rounded-xl border border-white/5 text-xs space-y-2 font-mono">
                <div className="flex justify-between">
                  <span className="text-[#becab9]">Date:</span>
                  <span className="text-white font-semibold">{selectedTx.date}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#becab9]">Time Stamp:</span>
                  <span className="text-white font-semibold">{selectedTx.time}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#becab9]">Flow Direction:</span>
                  <span className={`font-semibold capitalize ${selectedTx.type === 'income' ? 'text-[#78dc77]' : 'text-[#ff6b67]'}`}>
                    {selectedTx.type}
                  </span>
                </div>
              </div>
              ...
              <div className="flex space-x-2 pt-2">
                <button 
                  onClick={() => handleEditTransaction(selectedTx)}
                  className="flex-1 bg-white/5 hover:bg-white/10 border border-white/5 text-white py-3 rounded-xl font-bold flex items-center justify-center space-x-1.5 text-xs transition-all"
                >
                  <Icons.Edit />
                  <span>Edit</span>
                </button>
                <button 
                  ...
                  className="flex-1 bg-red-950/40 hover:bg-red-900/40 border border-red-500/30 text-red-200 py-3 rounded-xl font-bold flex items-center justify-center space-x-1.5 text-xs transition-all"
                >
                  <Icons.Trash />
                  <span>Delete</span>
                </button>
            ...
            <div className="bg-[#1c1b1b] w-full rounded-t-[30px] border-t border-white/10 p-6 space-y-4 max-h-[90%] overflow-y-auto animate-slideUp">
              ...
              <form onSubmit={handleSaveGoal} className="space-y-4">
                ...
                <div>
                  ...
                  <input 
                    type="text" 
                    placeholder="e.g. Setup Upgrade, Tech"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-[#2c2c2c] border border-white/10 rounded-xl py-3 px-4 text-white text-sm font-mono focus:outline-none focus:border-[#78dc77]"
                      required
                    />
                  </div>
                  <div>
